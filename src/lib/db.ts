    ...
    if (typeof (import.meta as any).glob === "function") {
      ...
      if (fs.existsSync(migrationsDir)) {
        ...
        for (const file of files) {
          migrations[`/migrations/${file}`] = fs.readFileSync(path.join(migrationsDir, file), "utf-8");
        }
      }
    }
...
export function ensureDbReady(): Promise<void> {
  ...
  return getSql().then(async () => {
    try {
      const { getDb } = await import("../server/db/kysely.ts");
      const db = getDb();
      const profile = await db.selectFrom("profiles").select("id").limit(1).executeTakeFirst();
      if (!profile) {
        const { seedDatabase } = await import("../server/seed/seed-runner.ts");
        await seedDatabase();
      }
    } catch {
      // Ignore if database/tables are not yet queryable
    }
  });
}
